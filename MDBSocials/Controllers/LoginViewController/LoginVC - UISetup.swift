...
extension LoginViewController {
    func setUpBackground() {
        ...
        friendView = UIImageView(frame: CGRect(x: 0, y: 0, width: view.frame.width, height: view.frame.height/3))
        friendView.center = CGPoint(x: view.frame.width/2, y: view.frame.height/4)
        friendView.image = UIImage(named: "convo")
        friendView.contentMode = .scaleAspectFit
        view.addSubview(friendView)
        
        titleLabel = UILabel(frame: CGRect(x: 0, y: 0, width: view.frame.width, height: view.frame.height/8))
        titleLabel.center = CGPoint(x: view.frame.width/2, y: friendView.frame.maxY + view.frame.height/16)
